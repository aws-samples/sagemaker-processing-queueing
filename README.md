...
```bash
sagemaker-queueing$ sam build --use-container
```
...
```bash
sagemaker-queueing$ sam logs -n StockCheckerFunction --stack-name sagemaker-queueing --tail
```
...
```bash
sagemaker-queueing$ pip install -r tests/requirements.txt --user
# unit test
sagemaker-queueing$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sagemaker-queueing$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name sagemaker-queueing
```
